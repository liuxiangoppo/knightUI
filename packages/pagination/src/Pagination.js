...
export default {
  ...
  watch: {
    ...
    pageSize (val) {
      if (val !== this.interPageSize) {
        // Todo this.handleChangePage(val)
      }
    }
  },
  computed: {
  ...
  render (h) {
    ...
    if (!this.simple) {
      ...
      pager = (
        <ul>
          <li><a class={prevClass} onClick={this.handlePrev}><i class="k-icon icon-previous"></i></a></li>
          {this._l(pagerList, page => page)}
          <li><a class={nextClass} onClick={this.handleNext}><i class="k-icon icon-next"></i></a></li>
        </ul>
      )
    } else {
      const prevClass = this.hasPrev() ? `button ${sizeClass}` : `button is-disabled ${sizeClass}`
